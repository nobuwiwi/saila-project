...
| 画像ストレージ | AWS S3（django-storages + boto3） |
...
### ログイン後のレイアウト（モバイル）
- サイドバーは非表示となり、ヘッダーの「ハンバーガーメニュー」からスライドイン（ドロワー）で表示される
- 名刺一覧は横スクロールのテーブルではなく、見やすい「カード形式」で縦に並ぶ
- 名刺詳細や各種モーダルは、画面幅を最大限（100%・マージン少し）使うように広がる
...
| 料金 | 無料プラン vs Proプラン（¥480/月）の2カラム比較 |
| CTA | 「無料で始める（30日間）」ボタン |
| フッター | © 2025 Saila |
